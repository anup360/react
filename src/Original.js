...
const moveField = (index,move)=>{

  const value = (move === "up")?index - 1 : index + 1;
...
const arrows = (index,len)=>{
  ...
  if(index === 0){
    return (
            <span>
              <button className="p-0 mx-1 btn btn-outline-primary" onClick={() => moveField(index,"down")}>
              <span className="k-icon k-i-arrow-down"></span>
                {/* <i className="fas fa-arrow-down" title="Move down"></i> */}
                </button>
            </span>
            )
  }
  ...
  if(index > 0 && index === (len - 1)){
    return (
            <span>
              <button className="p-0 mx-1 btn btn-outline-primary" onClick={() => moveField(index,"up")}>
              <span className="k-icon k-i-arrow-up"></span>
                {/* <i className="fas fa-arrow-up" title="Move up"></i> */}
                </button>
            </span>
            )
  }
...
const displayDropFields = (field,index)=>{
  ...
  return (<div key={field.id}>
            ...
            <div key={field.id} className="form-field position-relative my-2">
              ...
              { arrows(index,formFields.length) }
              
              <button className="p-0 mx-1 btn btn-outline-danger" onClick={() => handleDeleteField(field.id)}>
              <span className="k-icon k-i-delete"></span>
                {/* <i className="fas fa-close" title="delete field"></i> */}
              </button>
              
              <button className="p-0 mx-1 btn btn-outline-primary" onClick={() => handleFieldSetting(field.id)}><span className="k-icon k-i-edit"></span>
    ...
    return (
      <div style={{
        ...
        <label>
          ...
          {htmlControlMasterData?.length ? htmlControlMasterData.map(item=>(<>{<label><input type="radio" value={item.keyValue} /> { item.keyValue }</label>}</>)):<><label><input type="radio" value="Option 1"/> Option 1</label><label><input type="radio" value="Option 2"/> Option 2</label></>}
      </div>
    )
  }
